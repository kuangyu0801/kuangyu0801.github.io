...
- 遇到新问题且无法搜集相关资料怎么办, 讲述一次放弃的经历
- the most challenging problem, tell me about an important decision you made, what was the result
    + 5G
- tell me about a technique you recently learned, how do you get hands on it. 之后让我讲了讲具体的细节
- a time when you can't meet deadline, a time when you change the approach for the customer , 然后问了具体如何change的，想没想过alter solution.
- exceed expectation
- coolest thing from the work
    + 5G
- 我在实习中需要gather information and make a decision的例子
- 举一个例子，当你的peer在学业或者工作上陷入困难时，你是怎么做帮助他的，最后结果如何?
- 违背过的承诺
- 你以前实习的manager有没有对你失望过？楼主好说歹说展现了正能量，面试官追问：那后面manager就没有对你失望过了
- 遇到新问题且无法搜集相关资料怎么办
- 讲述一次放弃的经历
- 问怎么对待ddl
- 1自己面对的最大的挑战是什么？
...
- **Action**: 
	...
	- Performance for customer
		...
		+ collaborating closely with intra-team (SW higher layer, test engineering,  HW), external team(vendor, and operator) to resolve issue
	- Tight Deadline
...
- **
- **Situation**: 1st 5G Data Phone Call on a rush schedule before new year.
	- Keysight-MediaTek starts their first 5G NR Data Demo Camp in Beijing. Deadline are approaching for we only have 10 days to finish before the new year, and delay would result in next developping stage.
-  **Task**: 
	...
	- Branches are messy
	- Asking some in HQ to shelve a patch for here is almost impossible
...
|---|---|---|---|---|---|
|暑期實習/學生工|(DONE)<BR>WS20完成刷題三百+面試實習|SS21休學六個月實習|準備亞馬面試|完成碩論(找到老師是有跟公司合作的)|全職|
...
# Key Word 
前端後端框架先不用管，其實就是一個語言
DevOps是deploy到不同機器的work flow, 也先不用管
agile開發很簡單, 只需要去udacity或是udemy上面上一門課，面試遇到的話就問說接觸過就可以
SQL, NoSQL, Graph Database，要能夠說出簡單的架構，上網課就可以
我沒有學過也不知道的
...
- Open Source  Frameworks (z.B. Tomcat, Hibernate, Spring)
...
Minimum Qualifications
 ...
 - Currently has, or is in the process of obtaining, a BS or MS in
   Computer Science, Engineering, or a related technical discipline
   - ***Experience coding in at least one language*** (even if it is not one
   that Facebook uses) 
   - Ability to rapidly learn new development
   languages (***Python, PHP, C++, and Java are all in heavy use***) 
   - Ability to pick up new software, frameworks and APIs quickly 
   - Ability to perform 'guerrilla capacity planning' for internet service
   architectures 
   - Knowledge of ***basic large-scale internet service
   architectures (such as load balancing, LAMP, CDN's)***, even if you
   haven't worked on one 
   - Configuration and maintenance of common applications such as ***Apache, memcached, Squid, MySQL, NFS, DHCP, NTP,
   SSH, DNS, and SNMP*** 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1NTc2MzM1MzAsNTg2ODE1ODc1LDE4NT
